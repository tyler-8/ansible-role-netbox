...
Toggle `netbox_uwsgi_in_venv` to `true` if you want `uwsgi` to be installed in
the same virtual environment as NetBox. Otherwise it will install using the Python
version used to created the virtual environment (normal/legacy behavior).
